...
from schnetpack.transform import KNNRepresentation
...
def _atoms_collate_fn(batch):
	...
	coll_batch[structure.idx_m] = idx_m
	# KNN representation batching
	coll_batch[structure.knn] = torch.cat([d[structure.knn] for d in batch], dim=0)
	coll_batch[structure.knn_index] = [d[structure.knn].size(0) for d in batch]
